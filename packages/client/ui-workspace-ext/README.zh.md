...
工作区增强：输入框工具行的 Git 分支徽章（带锚定的分支切换菜单），以及运行在当前会话工作区目录里的右侧可收起终端侧栏。
...
**终端侧栏** 填充帧级浮层 `shell.overlay`。收起时是右缘全高竖条（同左侧栏的 rail 形态）；点击展开为 360px 面板，含启动/终止/清屏/收起控件、ANSI 清理后的输出区与命令输入行。终端是每进程一个的 bash 会话，通过 `subprocess.spawnTerminal` 在当前工作区目录启动；展开时增量轮询输出，收起时宿主保留有界缓冲。

node 半注册同源路由 `/api/workspace-ext`（`branch`、`branches`、`checkout`、`term/spawn|write|poll|kill|status`）。该能力可运行 git、可启动并喂给 shell，因此每个 handler 拒绝非回环 Host 头作为 DNS 重绑定防御。
