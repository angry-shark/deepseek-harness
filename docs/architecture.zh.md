...
[`dsh-base`](../packages/bundle/base/README.md) 是每个 profile 的第一层：模型适配器、工具、持久化、沙箱与审批策略、设置、凭据、遥测。[`dsh-web-app`](../packages/bundle/web-app/README.md) 增加浏览器应用；[`dsh-headless`](../packages/bundle/headless/README.md) 增加一次性运行器，且完全不带服务器；[`dsh-tui-app`](../packages/bundle/tui-app/README.md) 在同一个 base 之上增加全屏终端前端门面。
...
| 添加 UI 或编辑器集成 | 驱动 `ctx.agents` 并从 `session/event` 渲染；全屏终端前端门面是 [`dsh-tui`](../packages/ui/tui/README.md) |
