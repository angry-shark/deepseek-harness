...
`web`、`headless` 和 `tui` profile 首次使用时会从随附模板自动初始化（`web`：base + web-app；`headless`：base + headless；`tui`：base + dsh（CLI 自身的终端前端门面））。其他缺失的 profile 会显式报错，并提示运行 `dsh plugin --profile <name> add <package>`。
...
交互式终端界面（`dsh --profile tui`）要求 stdin 和 stdout 都是 TTY，通过核心注册表创建（或使用 `--resume <sessionId>` 恢复）一个持久化 Agent，并在用户退出前拥有终端屏幕。它不挂载 ApiProxy、Host、HTTP 服务器、Web 运行时或浏览器客户端。
