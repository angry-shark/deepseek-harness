...
The `web`, `headless`, and `tui` profiles auto-initialize from shipped templates on first use (`web`: base + web-app; `headless`: base + headless; `tui`: base + dsh (the CLI's own terminal front door)). Any other missing profile fails loud with a hint to run `dsh plugin --profile <name> add <package>`.
...
The interactive terminal surface (`dsh --profile tui`) requires TTY stdin and stdout, creates (or, with `--resume <sessionId>`, resumes) one persisted Agent through the core registry, and owns the terminal screen until the user exits. It mounts no ApiProxy, Host, HTTP server, Web runtime, or browser client.
