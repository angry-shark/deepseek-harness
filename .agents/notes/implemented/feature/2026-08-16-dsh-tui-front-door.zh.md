...
本次变更恢复交互式终端前端门面，作为新包 `@deepseek-ai/dsh-tui`（`packages/ui/tui`）以及随附 profile bundle `@deepseek-ai/dsh-tui-app`（`packages/bundle/tui-app`），而不是复活被移除的实现。旧代码面向已不存在的改名前 API（`dsh-user-interaction`、`dsh-compact`、`dsh-session-projection-cache`、`agent.send`/`agent.steer` 语义），其 2.9 万行代码捆绑的功能在当前不变量下需要重新设计；在现有接缝之上构建一个全新、聚焦的前端门面是更小、更可维护的基础。
...
`dsh-tui-app` 以编码 persona 和 `tui` 行叠加在 `dsh-base` 之上；`dsh-app-boot` 中的 `PROFILE_TEMPLATES` 和 `INSTALLATION_OWNED_PROFILE_TUPLES` 增加 `tui`，因此 `dsh --profile tui` 像 `web` 和 `headless` 一样自动初始化。包目录为 `tui-app`（与 `web-app` 一致），使包目录名在组间保持唯一，从而 tsconfig `paths` 通配符 `@deepseek-ai/dsh-*` 能无歧义地将 `@deepseek-ai/dsh-tui` 解析到 `packages/ui/tui`。

## Alternatives considered

- **复活被移除的 `packages/ui/tui`** —— 被拒绝：它是针对已移除 API 编写的，其恢复交接和 projection-cache 机制依赖不再存在的服务，沿用其决策需要从 2.9 万行已删除代码中重新推导当前不变量。
- **把前端门面构建进 `dsh-tui-app`** —— 被拒绝：前端门面是覆盖交互接缝的可复用产品界面，bundle 应承载组合，而不是第二个呈现实现。
...
- 前端门面为随附 profile 闭包增加 pi-tui 运行时依赖（外部化，通过扁平模块回退解析）。
- TUI 只渲染持久化日志记录的内容，且只通过交互接缝作答，因此其模型可见和持久化界面恰好是日志与服务已保证的内容。
- 多选问题、自定义文本答案、`@` 文件补全、会话选择器以及恢复时的工作目录重入暂缓，并在包 README 中记录。
